...
bool inode_block_operation(Disk* disk, Inode *inode, size_t block, uint8_t data[FS_BLOCK_SIZE], int operation_type) {
    ...
    if (block_ptr == 0x0 && operation_type==FS_BLOCK_READ) { // error: reading unallocated block 
		log_error("[fs] trying to read unallocated block\n");
		return false;
    } else if (block_ptr == 0x0 && operation_type==FS_BLOCK_WRITE && !is_indirect_ptr) { // allocating new direct block
    ...
    } else if (is_indirect_ptr) { // reading/writing/creating block with indirection
